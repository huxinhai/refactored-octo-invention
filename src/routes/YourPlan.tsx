...
import {Button, Card, ConfigProvider, Flex, Switch, Tag, Typography} from "antd";
...
const Plan = (props:PlanProps) => {
    ...
    return (
        <div className={'relative'}>
            {
                !tag || <div className={'absolute z-1 -top-7 left-25'}>
                    <Tag color="#34D399" className={'rounded-99999'}>{tag}</Tag>
                </div>
            }

            <Card className={`border-0 rounded-16 min-w-350 ${bg}`}>
                <Flex vertical>
                    <Title className={'mb20'} level={5}>{title}</Title>
                    ...
                    <Button className={'mt46'} size={'large'} style={{'--ant-button-default-color':'#000'} as  React.CSSProperties} block >{but}</Button>
                </Flex>
          </Card>
        </div>
...
const PlansInclude = (props:PlansIncludeProps) => {
    ...
    return (
        <Card className={'border-0 max-w-202'} style={{
            boxShadow: '0px 1px 2px 0px rgba(0, 0, 0, 0.05)'
        }}>
            <Flex vertical align={'center'}>
                <img src={svg} alt={title} className={'w-auto h24'} />
                <Title className={'mt16'}  level={5}>{title}</Title>
                <Text className={'mt8 text-center'} type={'secondary'}>{description}</Text>
            </Flex>
...
function RouteComponent() {
  return  <>
      ...
      <main className={'py80 px32'} style={{
          ...
          <ConfigProvider theme={{
              ...
              <Flex className={'mt35 gap-col-30px gap-row-15px'} justify={'center'} wrap >
                  {
                      PlanList.map((item)=><Plan {...item} key={item.title}/>)
                  }
              </Flex>
          </ConfigProvider>

          <Flex vertical justify={'center'} align={'center'} className={'mt50'}>
              <Title className={'mb32'} level={3}>All Plans Include</Title>
              <Flex wrap gap={32} justify={'center'}>
                  {
                      PlansIncludeList.map((item)=><PlansInclude {...item} key={item.title}/>)
                  }
              </Flex>
          </Flex>
      </main>
